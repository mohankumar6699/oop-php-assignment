...
$sql = "UPDATE   tblproduct SET name='{$_POST['name']}', code='{$_POST['code']}',  description='{$_POST['description']}', price='{$_POST['price']}', category='{$_POST['category']}' WHERE id='{$_POST['eid']}'";

if ($conn->query($sql) === TRUE) {
	

   echo '<p style="text-align: center; margin-top: 20px;">One Product Updated Successfully</p>';
	...
	<div class="w3-row-padding w3-margin-bottom" style="padding: 10px 50px;">
        ...
        ?>  
			...
			<p> <b>Product Name</b><br>
				<input type="text" name="name" placeholder="Name" value="<?php echo $row['name']; ?>" /></p>
			<p> <b>Product Description</b><br>
				<input type="text" name="description" placeholder="Description" value="<?php echo $row['description']; ?>" /></p>
			<p> <b>Code</b><br>
				<input type="text" name="code" placeholder="Please add unique key" value="<?php echo $row['code']; ?>" /> </p>
			<p> <b>Price</b><br>
				<input type="number" name="price" value="<?php echo $row['price']; ?>" /></p>
			<p> <b>Category</b><br>
				<select name="category">
					<option value="1" <?php if( $row['category'] == 1){ echo 'selected'; }?>>Movie</option>
					<option value="2" <?php if( $row['category'] == 2){ echo 'selected'; }?>>TV Series</option>
				</select></p>
